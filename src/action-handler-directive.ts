...
import { ActionHandler as ActionHandlerInterface, ActionHandlerElement } from './types';
...
customElements.define('action-handler-modernroomcard', ActionHandler);

const getActionHandler = (): ActionHandlerInterface => {
    const body = document.body;
    const existing = body.querySelector('action-handler-modernroomcard');
    if (existing) {
        return existing as ActionHandlerInterface;
    }

    const actionhandler = document.createElement('action-handler-modernroomcard');
    body.appendChild(actionhandler);

    return actionhandler as ActionHandlerInterface;
};

const actionHandlerBind = (element: ActionHandlerElement, options?: ActionHandlerOptions): void => {
    const actionhandler: ActionHandlerInterface = getActionHandler();
    if (!actionhandler) {
        return;
    }
