...
export const getValue = (entity: RoomCardEntity): string | number | undefined =>
    entity.attribute ? entity.stateObj?.attributes[entity.attribute] : entity.stateObj?.state;
...
export const getEntityIds = (config: RoomCardConfig): string[] => {
    ...
    config.rows?.forEach((row) => row.hide_if?.conditions?.forEach((condition) => entities.push(condition.entity)));
    (config.icon as RoomCardIcon | undefined)?.conditions?.forEach((condition) => entities.push(condition.entity));
    config.appearance?.states?.forEach((rule) => entities.push(rule.entity));
...
const getConditionEntitiesFromConfig = (config: RoomCardConfig): string[] => {
    ...
    const entities = [config.info_entities, ...rows];
...
export const checkConditionalValue = (item: EntityCondition, checkValue: unknown): boolean => {
    if (checkValue === undefined || checkValue === null) return false;
...
export const renderClasses = (config: RoomCardRow, classes?: string): string =>
